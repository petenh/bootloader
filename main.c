...
//        unsigned char ANSELBSave;//apr25
//        ANSELBSave = ANSELB; //apr25
//        ANSELBbits.ANSB4 = 0;   //Configure RB4/AN11 for digital input capability//apr25

//LATC = 0xff;//apr25
// ANSELC = 0x00; //apr25
// TRISCbits.TRISC6=1;//apr25

    eeprom_val = Read_b_eep(0);
    if (eeprom_val == 0x66) {
        program = 0x1;
        Write_b_eep(0, 0x33);
     }
//    if (sw2 == 0) program = 1; //apr25
    
    //Check Bootload Mode Entry Condition
//    	if((sw2 == 1) || (eeprom_val = 0x66))	//This example uses the sw2 I/O pin to determine if the device should enter the bootloader, or the main application code
	if(program == 0)	//This example uses the sw2 I/O pin to determine if the device should enter the bootloader, or the main application code
	{
//            mLED_3_Off();
    	//Restore default "reset" value of registers we may have modified temporarily.        
    //        ANSELB = ANSELBSave; //apr25

		_asm
		goto 0x2000			//If the user is not trying to enter the bootloader, go straight to the main application remapped "reset" vector.
		_endasm
	}
